...
def profile(con, columns: list[dict]) -> tuple[list[dict], list[dict]]:
    ...
    for col in columns:
        ...
        e = {
            "blank": f"count(*) FILTER (WHERE {c} IS NULL OR {c}='')",
            "nan": f"count(*) FILTER (WHERE isnan({n}))",
            "positive_infinity": f"count(*) FILTER (WHERE {n}='Infinity'::DOUBLE)",
            "negative_infinity": f"count(*) FILTER (WHERE {n}='-Infinity'::DOUBLE)",
            "non_numeric_nonblank": f"count(*) FILTER (WHERE {n} IS NULL AND {c} IS NOT NULL AND {c}<>'')",
            "finite": f"count(*) FILTER (WHERE isfinite({n}))",
            "negative_finite": f"count(*) FILTER (WHERE isfinite({n}) AND {n}<0)",
            "non_integral_finite": f"count(*) FILTER (WHERE isfinite({n}) AND {n}<>trunc({n}))",
    ...
    counts = con.execute("SELECT " + ",".join(f"count(*) FILTER (WHERE {c['id']} IS NOT DISTINCT FROM {lit(v) if v is not None else 'NULL'})" for c, v in zip(columns, candidates)) + ", count(*) FROM numeric_view").fetchone()
...
def audit_family(paths: list[Path], out: Path, memory: str = '4GB', threads: int = 4) -> dict:
    ...
    result = {'audit_version': VERSION, 'status': 'RUNNING', 'dataset': paths[0].parent.name, 'files': []}
