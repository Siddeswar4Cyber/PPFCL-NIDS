...
Issued 2026-09-08 after the [initial comparative review](../../reports/CP0-comparative-review.md). Category: CURRENT DESIGN DECISION / PLANNED, NOT IMPLEMENTED. A0 is a set of interfaces and testable candidates; it is not the final system. Initial model/FL/privacy/CL choices remain unvalidated. CP3 implementation note (2026-09-09): EXP-001/EXP-002 now implement and validate the audit and S1 membership contracts; they supply data evidence only. See [S1 protocol](../CP3-split-protocol-S1.md). CP4 follow-up RUN-002-P1 implements five fitted development preprocessing artifacts and verifies their numeric/role boundaries; see [P1](../CP4-preprocessing-P1.md). No detector or FL/DP/CL algorithm is empirically selected.
...
Every revision must record problem, hypothesis, comparison, acceptance rule, result, decision and new architecture version. A0 has no empirical supporting result and must not be described as validated.
